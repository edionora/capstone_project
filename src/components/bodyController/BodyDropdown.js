...
let dropdownButtons = currentBodyPart.subCategories.map((item, i) =>{
    return (
       <button type="button" className="btn dropdown-item pane-btn" onClick={()=> props.selectPart(item.id, item.title)}>
        {item.title}
        </button>  
    )
})
    return (
    <div className="btn-group dropdown pane-btn">
        <button className="btn btn-dark dropdown-toggle pane-btn"
                type="button"
